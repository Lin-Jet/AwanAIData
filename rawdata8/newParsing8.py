...
new_file = open("new_result_08.txt", "w")
...
for i, item in enumerate(yellow):
    ...
    tempoutput4 = tempoutput3.replace('\"', '\\"')
    tempoutput5 = tempoutput4.replace('\\n\\n', '\\n')
    tempoutput6 = tempoutput5.replace('。\\n', '。')
    output = tempoutput6.replace('。。', '。')


    new_file.write(f"\t{{\n\t\t\"instruction\": \"請介紹一下醫學家{yellow[i]}\",\n\t\t\"input\": \"\",\n\t\t\"output\": \"{output}\"\n\t}}")
    if i != len(yellow) - 1:
        new_file.write(",")
    new_file.write("\n")
